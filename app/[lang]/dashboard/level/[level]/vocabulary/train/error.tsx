...
export default function VocabularyTrainError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('Vokabel-Session konnte nicht gestartet werden:', error.digest ?? error.message)
  }, [error])
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-3xl bg-white p-12 text-center shadow-sm ring-1 ring-gray-900/5">
      ...
      <h1 className="text-3xl font-bold text-gray-900">{VOCABULARY_FALLBACKS.error_title}</h1>
      <p className="mt-3 text-xl text-gray-600">{VOCABULARY_FALLBACKS.error_description}</p>
      <button
        type="button"
        onClick={reset}
      ...
      >
        ...
        {VOCABULARY_FALLBACKS.error_retry}
      </button>
    </div>
  )
}
