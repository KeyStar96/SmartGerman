...
export const createSchema = (t: any) => z.object({
    personal: z.object({
        firstName: z.string().trim().min(2, t.registration.errors.firstname_required),
        lastName: z.string().trim().min(2, t.registration.errors.lastname_required),
        email: z.string().trim().email(t.registration.errors.email_invalid)
        ...
        street: z.string().trim().min(3, t.registration.errors.street_required),
        zip: z.string().trim().length(5, t.registration.errors.zip_length).regex(/^\d+$/, t.registration.errors.zip_numeric),
        city: z.string().trim().min(2, t.registration.errors.city_required),
        birthDate: z.string().trim().min(1, t.registration.errors.birthdate_required).regex(/^\d{2}\.\d{2}\.\d{4}$/, t.registration.errors.birthdate_incomplete || "Complete Date Required"),
    }),
});
